...
#TODO fix error (ExtensionFailed: Extension 'cogs.sound' raised an error: TypeError: 'function' object is not subscriptable)
    async def psound_autocompletion(interaction: discord.Interaction, current: str) -> typing.List[app_commands.choices[str]]:
        data=[]
        for v in os.listdir("sounds/*"):
            if v.endswith(".mp3") or v.endswith(".ogg"):
                sstr= v.split(".")
                
                data.append(app_commands.choices(sstr))
    ...
    @app_commands.autocomplete(sounds=psound_autocompletion)
    async def psound(self, interaction: discord.interactions.Interaction, sound: str):
        ...
        interaction.response.send_message(f"playing {sound}")
